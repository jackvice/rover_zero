...
def rover_main():
    ...
    #assert isinstance(envs.single_action_space, gym.spaces.Box), \  # vec env
        #"only continuous action space is supported"
...
def make_env_thunk(env_id, idx, capture_video, run_name, gamma):
    def thunk():
        env = gym.make('TurtleBot3_Circuit_Simple_Continuous-v0') 
        if capture_video and idx == 0:
            env = gym.make(env_id)
...
def advantages_and_returns(args, next_obs, agent, device, dones, rewards, values, next_done):
    """
...
class Args:
    ...
    capture_video: bool = True # False jmv
