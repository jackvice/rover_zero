...
class TurtleBot3Env(gym.Env, Node):
    def __init__(self, goal_list=None, max_env_size=None, continuous=True):
        ...
        #print('type(observation_space)', type(self.observation_space))
        #exit()
    ...
    def create_observation_space(self):
        ...
        imu_space = spaces.Box(low=-5, high=5, shape=(10,), dtype=np.float32)
    ...
    def calculate_movement_reward(self, prev_pos, current_pos, prev_theta, current_theta,
                                  forward_reward=1.0, direction_change_penalty=0.5):
        """
        Calculate the reward for movement efficiency and penalize frequent direction changes.
        
        :param prev_pos: Tuple (x, y) representing the previous position of the robot.
        :param current_pos: Tuple (x, y) representing the current position of the robot.
        :param prev_theta: Float representing the previous orientation of the robot in radians.
        :param current_theta: Float representing the current orientation of the robot in radians.
        :param forward_reward: The reward for moving forward.
        :param direction_change_penalty: The penalty for changing direction frequently.
        :return: A float value representing the movement efficiency reward.
        """
        # Calculate distance moved
        distance_moved = np.sqrt((current_pos.x - prev_pos.x)**2 + (current_pos.y - prev_pos.y)**2)
        
        # Calculate change in orientation
        orientation_change = abs(current_theta - prev_theta)
        
        # Normalize orientation change to [0, pi]
        orientation_change = min(orientation_change, 2*np.pi - orientation_change)
    
        # Reward for forward movement
        reward = distance_moved * forward_reward
    
        # Penalize for large orientation changes (indicative of turning or circling)
        if orientation_change > np.pi / 4:  # Arbitrary threshold for significant direction change
            reward -= direction_change_penalty * orientation_change / np.pi
        if  False: #self.num_timesteps % 1000 == 0:
            print('minus direction_change_penalty:', direction_change_penalty,
                  ', * orientation_change:',orientation_change,'/ np.pi')
    ...
    def calculate_reward(self, action, lidar_data, imu_data, previous_imu_state=None):
        reward = 0
        c_path_metric_multiplier = 0.03 # eg  /20
        stability_metric_multiplier = 3.0
        safe_distance_multiplier = 8.0
        ...
        movement_reward = self.calculate_movement_reward(self.previous_position, self.position,
                                                         self.prev_theta, self.theta,
                                                         forward_reward, direction_change_penalty)
        ...
        if previous_imu_state is not None:
            ...
            if  self.num_timesteps % 1000 == 0:
                print(#'clear_path:',round(clear_path_metric*c_path_metric_multiplier,4),
                      ...
                      '  movement reward:', round(movement_reward,4),
                      #self.safe_distance,
                      ' distance penility:',
