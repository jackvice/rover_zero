...
class TurtleBot3Env(gym.Env, Node):
    def __init__(self, goal_list=None, max_env_size=None, continuous=True):
        ...
        self.staying_in_threshold = 0.016 # Threshold for for staying in place depends on speed
    ...
    def step(self, action):
        ...
        if not self.observation_space.contains(state):
            print("Exiting Observation out of bounds with length:",len(state), ", and values:", state)
            exit()
    ...
    def navigationReward(self) -> float:
        ...
        The reward increases as the robot's heading aligns closer to the direct path towards the goal.
        The alignment is measured in radians, where a smaller difference between the robot's heading
        ...
        # Calculate the alignment score as a normalized value between 0 and 1, where 1 means perfect alignment
        # and 0 means the worst alignment. This is done by converting the heading's absolute value to a fraction
    ...
    def calculate_jitter_penalty(self):
        if self.latest_imu is None or self.previous_imu is None:
            ...
            return 0  # Cannot calculate jitter without both current and previous IMU data
        ...
        linear_acceleration_magnitude = np.linalg.norm([
            self.latest_imu.linear_acceleration.x - self.previous_imu.linear_acceleration.x,
            self.latest_imu.linear_acceleration.y - self.previous_imu.linear_acceleration.y,
            self.latest_imu.linear_acceleration.z - self.previous_imu.linear_acceleration.z
        ])
        ...
        if angular_acceleration_magnitude > self.angular_jitter_threshold:
            penalty -= 0.5  # Penalty value, adjust based on experimentation
            angular_jitter = True
        else:
        ...
        current_time = time.time()
        if False: #self.last_print_time is None or (current_time - self.last_print_time) > 60:
            #print(current_time, self.last_print_time)
            print("\n #################################################    Angular Jitter:",
                  angular_jitter,",   Linear Jitter:", linear_jitter )
            print("Angular jitter penalty, Thresh: ", self.angular_jitter_threshold , ',  Angular accel:',
                  angular_acceleration_magnitude)
            print("Linear jitter penalty, thresh:", self.linear_jitter_threshold, ',  Linear accel:',
    ...
    def setReward(self, state: np.ndarray, action) -> float:
        ...
        if self.get_goalbox:
            ...
            print("############## GOAL ACHIEVED, New Goal at x:",self.goal_x, ", y:",  self.goal_y )


            # Check for collision
        elif min(self.latest_scan) < self.collision_distance:
            collisionR = reward = self.reward_collision
        else:
        ...
        distance_reward = (delta_distance * 1000)
        ...
        if self.reward_print_counter % 100 == 0:
            print(#"\n heading", round(heading,3),
                #"\t nav R:", round(nav_reward,3),
                "\t jitter Rewd:", jitter_penalty,
                '\t deltaD R:', round(distance_reward,4),
                #'\t distance', self.goal_distance,
                #'\t in_placeR:', staying_in_place_penalty,
    ...
    def reset(self):
        ...
        # print("Reset state [360], heading", state[360], "Reset state [361], distance", state[361])
        ...
        if False: #not self.observation_space.contains(state):
            ...
            print(f"State out of bounds. Min state value: {np.min(state)}, Max state value: {np.max(state)}")
