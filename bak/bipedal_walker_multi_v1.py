...
class Agent(nn.Module): 
    def __init__(self, envs):
        ...
        # You can determine the exact output shape using a dummy input
        conv_out_dim = self._get_conv_out_dim((1, 3, 400, 600))

        # Fully connected layers
        self.fc = nn.Sequential(
            layer_init(nn.Linear(conv_out_dim + 24, 512)), 
            nn.ReLU(),
        )
    ...
    def forward_network(self, x):
        x_rgb = x[:, :, :, :-1] / 255.0  # Extract the original RGB channels excluding the added column
        conv_out = self.conv(x_rgb)

        x_telemetry = x[:, :, :, -1].mean(1).squeeze()  # Extract telemetry data (mean over channel dimension)
        combined = torch.cat([conv_out, x_telemetry], dim=1)
        return self.fc(combined)

    
    def Old_forward_network(self, x_rgb, x_telemetry):
        x_rgb = x_rgb / 255.0
        conv_out = self.conv(x_rgb)
        combined = torch.cat([conv_out, x_telemetry], dim=1)
        return self.fc(combined)
    ...
    def get_value(self, x):
        x_rgb, x_telemetry = x[:, :3], x[:, 3:]
        x_rgb = x_rgb.permute(0, 1, 4, 2, 3).reshape(-1, 3, 400, 600)
...
def main():
    ...
    ######### NEW
    # Assuming obs is your observation dictionary
    #print(torch.Tensor(next_obs['pixels']).shape)
    tensor = torch.Tensor(next_obs['pixels']).squeeze(0)
    print(tensor.shape)

    
    #exit()
    #pixels = torch.Tensor(next_obs['pixels']).permute(2, 0, 1).unsqueeze(0).to(device)  # (1, 3, 400, 600)
    
    #  squeeze(0): Removes the extra leading dimension, resulting in (400, 600, 3).
    #  permute(2, 0, 1): Rearranges the dimensions to (3, 400, 600).
    #  unsqueeze(0): Adds the batch dimension back, resulting in (1, 3, 400, 600).
    pixels = torch.Tensor(next_obs['pixels']).squeeze(0).permute(2, 0, 1).unsqueeze(0).to(device)  # (1, 3, 400, 600)



    state = torch.Tensor(next_obs['state']).unsqueeze(-1).unsqueeze(-1).to(device)    # (24, 1, 1)
    state = state.view(1, 24, 1, 1).expand(-1, -1, 400, 600)
    #state = state.expand(-1, 400, 600).unsqueeze(0)
    # Concatenate
    next_obs = torch.cat([pixels, state], dim=1)
    ########## end new
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            ######### NEW
            # Assuming obs is your observation dictionary
            pixels = torch.Tensor(next_obs[0]['pixels']).permute(2, 0, 1).unsqueeze(0).to(device)  # (1, 3, 400, 600)
            state = torch.Tensor(next_obs[0]['state']).unsqueeze(-1).unsqueeze(-1).to(device)    # (24, 1, 1)
            state = state.expand(-1, 400, 600).unsqueeze(0)
            # Concatenate
            next_obs = torch.cat([pixels, state], dim=1)
            ########## end new
        ...
        # flatten the batch
        b_obs = obs.reshape((-1,) + (num_frames , frame_height, frame_width, 3)) 
