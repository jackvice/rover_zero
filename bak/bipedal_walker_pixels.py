...
def main():
    ...
    obs = torch.zeros((args.num_steps, args.num_envs) + (4 , 400, 600, 3)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        # flatten the batch
        b_obs = obs.reshape((-1,) + (4 , 400, 600, 3)) # envs.single_observation_space.shape)
...
class AgentOld(nn.Module): # (3, 600, 400)
    def __init__(self, envs):
        super().__init__()
        self.network = nn.Sequential(
            # Adjusted for 12 channels, 4 stacked RGB images
