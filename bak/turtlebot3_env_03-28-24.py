...
def flatten_observation(obs_dict):
    lidar_flat = obs_dict['lidar'].flatten()
    imu_flat = obs_dict['imu'].flatten()
    goal_vector_flat = obs_dict['goal_vector'].flatten()
    return np.concatenate([lidar_flat, imu_flat, goal_vector_flat])
...
class TurtleBot3Env(gym.Env, Node):
    def __init__(self, goal_list=None, max_env_size=None, continuous=True):
        ...
        self.observation_size = 24 # 12 lidar + 9 imu + heading + distance
        self.action_size = 2
        ...
        self.previous_imu_state = np.zeros(10)
        self.max_timesteps = 750
    ...
    def seed(self, seed=None):
        ...
        # If there are other stochastic elements or submodules, seed them here.
        # Example:
        # self.some_other_module.seed(seed)
        return [seed]
    ...
    def step(self, action):
        ...
        info = {}
        #print("####################step obs type", type(obs_dict),'\n')
    ...
    def noise_reduce_scan(self, scan_data):
        ...
        return reduced_scan #noisy_scan
    ...
    def getState(self):
        ...
        # Retrieve IMU data
        # Check if new IMU data is received
        if self.new_imu_received:
            imu_data = self.get_imu_data()
            # Reset the flag after processing the new data
            self.new_imu_received = False
        else:
            # Handle the case where new data isn't available (e.g., use the last known data or default values)
            imu_data = self.previous_imu_state  # or some default
    ...
    def calculate_reward(self, action, obs_next, previous_imu_state=None):
        ...
        if current_distance < self.goalbox_distance:
            print('\n################################################# Goal!!  goal x',
                  self.goal_x, 'y', self.goal_y,',  postiion x', self.position.x, ', y', self.position.y)
        ...
        if  self.num_timesteps % 500 == 0:
            print('\n', "no noise or stability"
                  ...
                  #'  +smoothness_metric:', round(smoothness_metric * smoothness_metric_multiplier,4),
                  #'  +stability:', round(stability_metric*stability_metric_multiplier,4),
    ...
    def reset(self, **kwargs):
        ...
        print("############### rest sim,  goal_x:", self.goal_x, 'and goal_y:', self.goal_y)
        self.position = Point(x=-2.0, y=-0.5, z=0.0)
        self.previous_position = Point(x=-2.0, y=-0.5, z=0.0)
        self.theta = 0 # yaw in radians
        self.prev_theta = 0 # yaw in radians
    ...
    def get_imu_data(self):
        if self.latest_imu is None:
            return np.zeros(10)  # Return zeros if no IMU data is available
