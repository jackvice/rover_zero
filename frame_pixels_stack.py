...
class LazyFrames:
    ...
    def __init__(self, frames: list, lz4_compress: bool = False):
        ...
        #print(frames)
        #exit()
        #frame = frames[0]#['pixels']  # Adjust this line to get the 'pixels' key
...
class FrameStack(gym.ObservationWrapper):
    ...
    Note:
        ...
        - The observation space must be :class:`Box` type. If one uses :class:`Dict`
          ...
          - After :meth:`reset` is called, the frame buffer will be filled with the initial 
            observation. I.e. the observation returned by :meth:`reset` will consist 
            of ``num_stack`-many identical frames,
    ...
    def __init__(
        ...
        if isinstance(self.observation_space, gym.spaces.Dict):
            # Handling environments that produce dictionary observations
            low = np.repeat(self.observation_space['pixels'].low[np.newaxis, ...], num_stack, axis=0)
            high = np.repeat(self.observation_space['pixels'].high[np.newaxis, ...], num_stack, axis=0)
            self.observation_space = gym.spaces.Dict({
                'pixels': Box(low=low, high=high, dtype=self.observation_space['pixels'].dtype)
            })
        else:
    ...
    def observation(self, observation):
        frames = [frame['pixels'] for frame in self.frames]  # Extract the 'pixels' key from each frame
        return {'pixels': LazyFrames(frames, self.lz4_compress)}  # Return a dictionary with 'pixels' key
    ...
    def step(self, action):
        ...
        self.frames.append(observation)  # This remains the same
    ...
    def reset(self, **kwargs):
        ...
        [self.frames.append(obs) for _ in range(self.num_stack)]  # This remains the same
