...
def log_episode_info(infos, writer, global_step):
    """
    Logs episodic information if available.

    Parameters:
    infos (dict): Information dictionary from the environment step.
    writer (Writer): Used for logging scalar values.
    global_step (int): The global step count for logging purposes.
    """
    print('   Before if, in log infos:', infos)
    if "final_info" in infos:
        print('in log infos:', infos)
...
def calculate_advantages_and_returns(args, next_obs, agent, device, dones, rewards, values):
    """
...
def process_minibatch(agent, b_inds, args, b_obs, b_actions, b_logprobs, b_advantages,
                      b_returns, b_values, optimizer):
    """
    Processes a mini-batch and updates the agent.

    Parameters:
    agent (Agent): The agent performing actions and value estimations.
    b_inds (array): Indices for the current mini-batch.
    args (Namespace): Training arguments and hyperparameters.
    b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_values (Tensor): Batched tensors.
    optimizer (Optimizer): The optimizer for the agent.
    
    Returns:
    float: The approximate KL divergence for this mini-batch.
    float: The clip fraction for this mini-batch.
    Tensor: The loss for this mini-batch.
    """
    _, newlogprob, entropy, newvalue = agent.get_action_and_value(
        b_obs[b_inds], b_actions.long()[b_inds]
    )
    logratio = newlogprob - b_logprobs[b_inds]
    ratio = logratio.exp()

    with torch.no_grad():
        # Calculate approx_kl and clipfracs http://joschu.net/blog/kl-approx.html
    ...
    if args.clip_vloss:
        v_loss_unclipped = (newvalue - b_returns[b_inds]) ** 2
        v_clipped = b_values[b_inds] + torch.clamp(
            newvalue - b_values[b_inds], -args.clip_coef, args.clip_coef
        )
        v_loss_clipped = (v_clipped - b_returns[b_inds]) ** 2
        v_loss_max = torch.max(v_loss_unclipped, v_loss_clipped)
        v_loss = 0.5 * v_loss_max.mean()
    else:
        v_loss = 0.5 * ((newvalue - b_returns[b_inds]) ** 2).mean()

    # Calculate total loss and update agent
    ...
    return approx_kl, clipfrac, loss


def process_epoch(args, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_values, agent, optimizer):
    """
    ...
    Returns:
    bool: True if early stopping due to target KL divergence, otherwise False.
    """
    ...
    for start in range(0, args.batch_size, args.minibatch_size):
        ...
        approx_kl, clipfrac, _ = process_minibatch(
            agent, mb_inds, args, b_obs, b_actions, b_logprobs, b_advantages, b_returns, b_values, optimizer
        )
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        """
        global_step, rewards, values, obs, next_obs, actions, logprobs =  \
            perform_environment_steps(next_obs, next_done,args, envs, agent, device, writer, global_step,
        ...
        # flatten the batch
        b_obs, b_logprobs, b_actions, b_advantages, b_returns, b_values = flatten_batches(envs, obs, logprobs,
                                                                                          actions, advantages,
                                                                                          returns, values)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                approx_kl, clipfrac, loss = process_minibatch(agent, b_inds, args, b_obs, b_actions,
                                                              b_logprobs, b_advantages,
                                                              b_returns, b_values, optimizer)
                """
