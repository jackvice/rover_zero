...
def frame_stacker(frame_queue):
    # Ensure the deque contains 4 frames
    ruturn np.zeros((1, 4, 84, 84))
    #while len(frame_queue) < 4:
    #    frame_queue.append(np.zeros((1, 1, 84, 84)))
...
if __name__ == "__main__":
    ...
    xz = frame_stacker(stacked_frames)
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        writer.add_scalar("losses/explained_variance", explained_var, global_step)
        print("SPS:", int(global_step / (time.time() - start_time)))
              #'best_reward', best_reward, ',  rewards sum', torch.sum(rewards))
        writer.add_scalar("charts/SPS", int(global_step / (time.time() - start_time)), global_step)
